<?php
 /**
   ...
   *
   * $Id: ganttchart.php 1773 2006-07-22 23:01:26Z sandy $
   */
...
$querydep->addJoin('events', '' ,'events.id=phase.eventsid', FALSE);
...
$querydep->addCondition("events.id='".$eventsid."'");
...
// Select the PHASES which belong to the events
$queryphase = &$db->createQuery();
$queryphase->addTable('events');
$queryphase->addJoin('phase', '', 'events.id=phase.eventsid', FALSE);
$queryphase->addField('id', ' ', 'phase', 'phase_');
$queryphase->addField('name', ' ', 'events', 'events_');
$queryphase->addField('name', ' ', 'phase', 'phase_');
$queryphase->addField('current_planning', ' ', 'phase', 'phase_');
$queryphase->addField('startdate', ' ', 'events', 'events_');
$queryphase->addField('phase.startdate AS phase_startdate');
$queryphase->addField('phase.enddate AS phase_enddate');
$queryphase->addCondition("events.id='".$eventsid."'");
...
// Select the TIMES that have been booked on the events
...
$querybooked->addCondition("phase.eventsid='".$eventsid."'");
...
// Select the TIMES that have been planned on the events
$queryplanned  = &$db->createQuery();
$queryplanned->addTable('events');
$queryplanned->addJoin('phase', '', 'events.id=phase.eventsid', FALSE);

$queryplanned->addField('id', ' ', 'phase', 'phase_');
$queryplanned->addField('events.name AS events_name');
...
$queryplanned->addCondition("events.id='".$eventsid."'");
addDefaultConditions($queryplanned);

$queryplanned->addGroupBy("events.id");
...
//$graph->title->Set(atktext('resource_planning_eventssurvey').': '.$dbrecordsphase[0]['events_name']);
...
// milestones
$milestonenode = &atkGetNode("events.deliverable");
$deliverables = $milestonenode->selectDb("events_id=".$eventsid, "duedate");
...
//Add only a vertical line with the actual date when this actual date is between the start- and enddate of the events
$startdateevents = "";
$enddateevents = "";
foreach($gant as $phase)
{
  if(isset($phase["startdate"]) && ($phase["startdate"]<$startdateevents || $startdateevents=="")) $startdateevents = $phase["startdate"];
  if(isset($phase["enddate"]) && ($phase["enddate"]>$enddateevents || $enddateevents=="")) $enddateevents = $phase["enddate"];
}

if($startdateevents <= (date("Y-m-d")) AND $enddateevents >= (date("Y-m-d")))
{
